...
#env = make_parallel_env("simple_spread", 3, seed, True)
...
class Critic(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(num_state*4, 100)              # 用的是shared_critic 所以输入的维度是num_state*agent_num
...
class PPO():
    ...
    def __init__(self, agent_i):          # 加一个编号agent_i，用来区别不同agent
        ...
        self.writer = SummaryWriter('../exp/run6')
    ...
    def select_action(self, state):                 # 目前这个select_action只支持一个环境一个agent的状态输入[num_state, ]
        state = torch.from_numpy(state).float().unsqueeze(0)  # [1, num_state]  理论上可以改成支持多个环境一个agent的输入 即[env_num, num_state]
    ...
    def save_param(self):               # 保存模型参数
        #import pdb; pdb.set_trace()
        torch.save(self.actor_net.state_dict(), '../param/net_param/actor_net_%i' % self.agent_i + str(time.time())[:10] +'.pkl')
        torch.save(self.critic_net.state_dict(), '../param/net_param/critic_net_%i' % self.agent_i + str(time.time())[:10] +'.pkl')
    ...
    for i_epoch in range(2000):
        for j in range(env_num):     # 依次运行仿真环境
            ...
            for t in count():
                ...
                next_state, reward, done, _ = env.step([one_hot_action1, one_hot_action2, one_hot_action3, one_hot_action4])
                ### 把得到的结果分别存成每个agent的trans  因为用的是shared_critic，所以state都存一样的，但更新actor网络时只用自己的state[i]，见update中的说明
                trans1 = Transition(np.concatenate((state[0],state[1],state[2],state[3])), action1, action_prob1, reward[0], next_state[0])
                trans2 = Transition(np.concatenate((state[0],state[1],state[2],state[3])), action2, action_prob2, reward[1], next_state[1])
                trans3 = Transition(np.concatenate((state[0],state[1],state[2],state[3])), action3, action_prob3, reward[2], next_state[2])
                trans4 = Transition(np.concatenate((state[0],state[1],state[2],state[3])), action4, action_prob4, reward[3], next_state[3])
                if render: env.render()
                agent1.store_transition(trans1)
                agent2.store_transition(trans2)
                agent3.store_transition(trans3)
                agent4.store_transition(trans4)
                state = next_state
                ### 每个环境做episode_num次交互，做完就进入下一个env
